...
補足（運用に必須）:
- [+] URLクエリ（`?date=YYYY-MM-DD`）で日付を全画面に反映（マッパー/タイムバー等）
...
## フェーズ5：Excel取込（1〜3日）
- [+] `.xlsx` 取込（ブラウザ）
- [+] 日付行/氏名列/日付列範囲/データ範囲を指定して取込
- [+] ひと月分の列をまとめて読み込み（検出した日付列すべてを更新）
- [+] 勤務コード抽出（A〜F / 時刻レンジ / 固定マーク）→ `shifts` upsert
- [+] 取込プレビュー（氏名の手動紐づけ）
- [+] 監査ログ記録（import）
