...
- **退勤30分前以降は休憩開始不可**
- **権限**: 管理者のみ編集、主任は閲覧のみ（ログインなし／PINで編集付与）
- **同時接続**: 最大3台想定
...
- **PIN検証（秘匿必須）**: Firebase Cloud Functions（Callable）でPIN照合→**編集セッション発行**
- **ホスティング**: Vercel もしくは Firebase Hosting（無料枠）

> ログイン無しで「編集者だけ制限」を実現するため、PINをクライアント側で完結させない（DBに平文で置かない）。Cloud Functionsで検証し、短命セッションで書き込み権限を通す。
